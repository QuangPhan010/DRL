...
export default function ActivityDetail() {
  ...
  const handleCheckIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Check-in định vị GPS thành công!");
  ...
  const handleCheckOut = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Check-out định vị GPS thành công!");
  ...
  const getCheckInStatus = (act: any) => {
    if (act?.check_in_time && !act?.selfie_resubmit_requested) {
    ...
    if (!start || !end) return { enabled: true, text: "Check-in GPS" };
    ...
    return { enabled: true, text: act?.selfie_resubmit_requested ? "Bổ sung Selfie" : "Check-in GPS" };
  };
  ...
  const getCheckoutStatus = (act: any) => {
    ...
    const end = parseDateTime(act?.date, act?.end_time);
    if (!end) return { enabled: true, text: "Check-out GPS" };
    ...
    return { enabled: true, text: "Check-out GPS" };
  };
  ...
  return (
    <div className="space-y-6 pb-24">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-muted/30">
                        ...
                        <TableHead className="text-center">Check-in GPS</TableHead>
                        <TableHead className="text-center">Check-out GPS</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
      ...
      {/* Dialog: Check-in GPS Simulation */}
      ...
      {/* Dialog: Check-out GPS Simulation */}
